    ...
    <Secao id="recursos" className="z-20 py-24 lg:py-32">
      ...
      <div className="mt-14 grid gap-12 lg:grid-cols-[minmax(0,23rem)_1fr] lg:gap-16">
        ...
        <div key={grupo.nome} className="painel-recursos" aria-live="polite">
          ...
          <h3 className="titulo mt-3 t-sub text-text0">{grupo.nome.replace(' · em breve', '')}</h3>
          <p className="mt-3 t-corpo text-gold200">{grupo.resumo}</p>
          <p className="mt-3 t-apoio text-text1">{grupo.beneficio}</p>
          <dl className="mt-8 grid gap-x-8 sm:grid-cols-2">
            {grupo.itens.map(([titulo, texto], i) => (
              <div key={titulo} className="painel-recursos__item border-t border-line py-6" style={{ animationDelay: `${i * 70}ms` }}>
                <dt className="titulo text-[1.1rem] text-text0">{titulo}</dt>
