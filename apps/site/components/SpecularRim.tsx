...
// React Bits SpecularButton: um renderer para todos os CTAs da landing.
...
void main(){
 ...
 float r=min(uRadius*uDpr,min(uHalfSize.x,uHalfSize.y));
...
function targets() {
  for (const e of entries) {
    ...
    if (pointer && distance === 0 && !reduced()) {
      const nx = (pointer.x-rect.left-rect.width/2)/(rect.width/2);
      const ny = (rect.top+rect.height/2-pointer.y)/(rect.height/2);
      e.target = Math.atan2(2/rect.height,-2/rect.width)+nx*.3+ny*.15;
    }
  }
...
function paint(now: number) {
  ...
  for (const e of entries) {
    ...
    program.uniforms.uRadius.value=parseFloat(getComputedStyle(e.host).borderTopLeftRadius) || 0;
  ...
  useEffect(() => {
    ...
    try {
      if (!renderer) {
        ...
        program=new Program(renderer.gl,{vertex,fragment,uniforms:{uHalfSize:{value:[1,1]},uCenter:{value:[0,0]},uAngle:{value:2.4},uBright:{value:0},uDpr:{value:1},uRadius:{value:8},uColor:{value:[1,.88,.65]}}});
